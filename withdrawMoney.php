...
$userid = $_SESSION["userId"];
	...
	$sql = "SELECT balance FROM user where userid = '$userid'";
	    ...
	    while($row = $result->fetch_assoc()) {
			...
			{
				echo "<br><h1>Your balance is not enough for withdraw..</h1>";
			}
			else
			{
	    ...
	    while($row = $result->fetch_assoc()) {
			$Email = $row["user_id"];		
					...
					$sql1 = "INSERT INTO transactions ( userid, transaction_type) VALUES ( '$userid', 'WITHDRAW');";
					...
					$sql1 = "INSERT INTO withdraw_deposit ( amount,payment_date,is_withdraw,is_deposit, transaction_id,userid,retailer_id) VALUES ( '$amount', now() , 'yes' , 'no' , '$transactionIdTobeInserted','$userid','$Email');";
					
					if(mysqli_query($conn, $sql1)){
					    echo "withdraw  table value added successfully.";
